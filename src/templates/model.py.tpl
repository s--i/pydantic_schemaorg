...
# Pydantic v2: import ConfigDict and Field at module scope so generated models use model_config
from pydantic import Field, ConfigDict
{% if model.uses_validators %}
...
class {{ model.valid_name }}({{ model.parents | sort(attribute='depth', reverse=True) | map(attribute='valid_name') | join(', ') }}):
    ...
    model_config = ConfigDict(
        # Add repo-level defaults here as required, for example:
        # extra = "ignore",
